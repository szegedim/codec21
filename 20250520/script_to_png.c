...
export HTML_PATH="https://www.theme25.com/$(uuidgen | sha256sum | cut -d' ' -f1).tig?Content-Type=text/html"; export PIPE_PATH="https://www.theme25.com/$(uuidgen | sha256sum | cut -d' ' -f1).tig?Content-Type=text/plain"; export STDIN_PATH="https://www.theme25.com/$(uuidgen | sha256sum | cut -d' ' -f1).tig?append=1"; export STDOUT_PATH="https://www.theme25.com/$(uuidgen | sha256sum | cut -d' ' -f1).tig?Content-Type=image/png"; printf "$STDIN_PATH\n$STDOUT_PATH\n" | curl -X PUT --data-binary @- "$PIPE_PATH"; echo; echo pipe $PIPE_PATH; cat index.html | curl -X 'PUT' --data-binary @- "$HTML_PATH" | echo open $HTML_PATH#$PIPE_PATH; gcc script_to_png.c -o script_to_png.out -I/usr/include/freetype2 -lpng -lfreetype -lcurl -lpthread -lz -lm && ((while true; do curl "$STDIN_PATH&take=1" | ./typer; sleep 1; done) | ./my_script.sh | ./script_to_png.out "$STDOUT_PATH")
